	...
	<script>
		...
		/*
			...
			$.ajax({
				...
				dataType: 'text',	         // 수신할 데이터타입(text, xml, json)
				success: function(result) {
					console.log(result); 	//수신된 데이터
				},
				error: function(a,b,c) {
					console.log(a,b,c)l		//예외처리
				}
			});
			...
			$.ajax({
				...
				success: function(result) {
					...
					$(result).find('user').each((index,item)=>{
						...
						temp += `<li>암호: \${$(item).find('id').text()}</li>`;
						temp += `<li>이름: \${$(item).find('pw').text()}</li>`;
						temp += `<li>레벨: \${$(item).find('lv').text()}</li>`;
						temp += '</ul>';
